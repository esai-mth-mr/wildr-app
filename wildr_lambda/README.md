...
```bash
wildr_lambda$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
wildr_lambda$ sam local start-api
wildr_lambda$ curl http://localhost:3000/
```
...
```bash
wildr_lambda$ sam logs -n HelloWorldFunction --stack-name wildr_lambda --tail
```
...
```bash
aws cloudformation delete-stack --stack-name wildr_lambda
```
