function getDaysInMonth(month,year) {
    ...
    let curMon = new Date(year,month),
        nextMon = new Date(year,month+1);
    return Math.floor((nextMon.getTime()-curMon.getTime())/(24*3600*1000));
}
export function getDateTimeSince(target) { // target should be a Date object
    let now = new Date(), yd, md, dd, hd, nd, sd, out = [];
