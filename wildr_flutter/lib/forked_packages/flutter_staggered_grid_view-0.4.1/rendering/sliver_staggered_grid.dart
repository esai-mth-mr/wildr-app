...
import 'package:wildr_flutter/forked_packages/flutter_staggered_grid_view-0.4.1/rendering/sliver_variable_size_box_adaptor.dart';
...
bool _nearEqual(double d1, double d2) => (d1 - d2).abs() < _epsilon;
...
class SliverStaggeredGridGeometry {
  /// Creates an object that describes the placement
  /// of a child in a [RenderSliverStaggeredGrid].
  ...
  @override
  String toString() => 'SliverStaggeredGridGeometry('
...
class RenderSliverStaggeredGrid extends RenderSliverVariableSizeBoxAdaptor {
  ...
  void performLayout() {
    ...
    // If the viewport is resized, we keep the in memory
    // the old offsets caches. (Useful if only
    // the orientation changes multiple times).
    ...
    // A staggered grid always have to layout the
    // child from the zero-index based one to the last visible.
    ...
    for (var index = viewportOffset.firstChildIndex;
      ...
      if (geometry == null) {
        // There are either no children, or we
        // are past the end of all our children.
        reachedEnd = true;
        break;
      }
      ...
      if (child != null) {
        final childParentData =
            child.parentData! as SliverVariableSizeBoxAdaptorParentData
              ..layoutOffset = geometry.scrollOffset
              ..crossAxisOffset = geometry.crossAxisOffset;
  ...
  /// Finds the first available block with at least
  /// the specified [crossAxisCount] in the [offsets] list.
  static _Block _findFirstAvailableBlockWithCrossAxisCount(
    int crossAxisCount,
    List<double> offsets,
  ) =>
      _findFirstAvailableBlockWithCrossAxisCountAndOffsets(
        crossAxisCount,
        List.from(offsets),
      );

  /// Finds the first available block
  /// with at least the specified [crossAxisCount].
  ...
  static _Block _findFirstAvailableBlock(List<double> offsets) {
    ...
    // We have to use the _nearEqual
    // function because of floating-point arithmetic.
...
/// See also:
///
///  * [StaggeredGridView], which can use thi
///  s delegate to control the layout of its
///    tiles.
///  * [SliverStaggeredGrid], which can use
///  this delegate to control the layout of its
///    tiles.
///  * [RenderSliverStaggeredGrid], which can use
///  this delegate to control the layout of
///    its tiles.
...
///  * [StaggeredGridView], which can use this
///  delegate to control the layout of its
///    tiles.
///  * [SliverStaggeredGrid], which can use this
///  delegate to control the layout of its
///    tiles.
///  * [RenderSliverStaggeredGrid], which can use
///  this delegate to control the layout of
...
///
///  * [SliverStaggeredGridDelegate], which creates staggered grid layouts.
///  * [StaggeredGridView], which can use this delegate
///  to control the layout of its
///    tiles.
///  * [SliverStaggeredGrid], which can use this delegate
///  to control the layout of its
///    tiles.
///  * [RenderSliverStaggeredGrid], which can use this delegate
