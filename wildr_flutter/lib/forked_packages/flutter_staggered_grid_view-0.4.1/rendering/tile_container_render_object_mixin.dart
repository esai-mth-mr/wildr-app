...
mixin TileContainerRenderObjectMixin<ChildType extends RenderObject,
    ...
    assert(() {
      if (child is! ChildType) {
        throw FlutterError(
            ...
            'example, a RenderSliver cannot'
            ' be the child of a RenderBox because '
            'a RenderSliver does not understand '
            'the RenderBox layout protocol.\n'
            '\n'
            'The $runtimeType that expected a '
            '$ChildType child was created by:\n'
            '  $debugCreator\n'
            '\n'
            'The ${child.runtimeType} that did not match'
  ...
  void attach(PipelineOwner owner) {
    super.attach(owner);
    for (final child in _childRenderObjects.values) {
      child.attach(owner);
    }
  }
