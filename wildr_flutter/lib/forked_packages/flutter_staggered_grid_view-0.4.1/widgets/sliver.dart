...
import 'package:wildr_flutter/forked_packages/flutter_staggered_grid_view-0.4.1/rendering/sliver_staggered_grid.dart';
...
///
/// Helps subclasses build their children
/// lazily using a [SliverVariableSizeChildDelegate].
...
/// An element that lazily builds children
/// for a [SliverVariableSizeBoxAdaptorWidget].
///
/// Implements [RenderSliverVariableSizeBoxChildManager],
/// which lets this element manage
...
class SliverVariableSizeBoxAdaptorElement extends RenderObjectElement
  ...
  // In both cases, we cache the results of calling into
  // our delegate to get the widget,
  ...
  Widget? _build(int index) => _childWidgets.putIfAbsent(
        index,
  ...
  void didAdoptChild(RenderBox child) {
    assert(_currentlyUpdatingChildIndex != null);
    (child.parentData! as SliverVariableSizeBoxAdaptorParentData).index =
  ...
  void visitChildren(ElementVisitor visitor) {
    // The toList() is to make a copy so that the
    // underlying list can be modified by
    // the visitor:
...
///
/// [SliverStaggeredGrid] places its children
/// in arbitrary positions determined by
...
///    except that it uses a prototype list
///    item instead of a pixel value to define
...
class SliverStaggeredGrid extends SliverVariableSizeBoxAdaptorWidget {
  ...
  ///
  /// Uses a [SliverStaggeredGridDelegateWithFixedCrossAxisCount]
  /// as the [gridDelegate],
  ...
  /// See also:
  ///
  ///  * [StaggeredGridView.count], the equivalent constructor for
  ...
  ///
  /// Uses a [SliverStaggeredGridDelegateWithMaxCrossAxisExtent]
  /// as the [gridDelegate],
  /// and a [SliverVariableSizeChildListDelegate] as the [delegate].
  ///
  /// See also:
  ///
  ///  * [StaggeredGridView.extent], the equivalent constructor
  ///  for [StaggeredGridView] widgets.
