// ignore_for_file: lines_longer_than_80_chars, only_throw_errors
...
/// Manager which works in Isolate, respond on
/// [IsolateBlocEvent]s from UI Isolate,
/// manages [IsolateBlocBase]s and
/// implements [register] and [getBloc] functions.
class IsolateManager {
  /// Creates isolate manager and set [instance].
  ///
  /// Don't forget to call [initialize] to
  /// subscribe on messages and call [Initializer].
  factory IsolateManager({
    required IIsolateMessenger messenger,
  }) =>
      instance = IsolateManager._internal(messenger);
  ...
  final _isolatedBlocWrappers =
      <IsolateBlocBase, List<WildrGqlIsolateBlocWrapper>>{};

  final _initializeCompleter = Completer();
  StreamSubscription<IsolateEvent>? _serviceEventsSubscription;

  /// Finish initialization by calling [Initializer]
  /// and sends initial states to the [UIIsolateManager].
  ///
  /// Throws [InitializerException] when some exception
  /// is thrown in [Initializer] in debug mode.
  ...
  WildrGqlIsolateBlocWrapper<S>
    ...
    late WildrGqlIsolateBlocWrapper<S> wrapper;
    B? isolateBloc;
    ...
    return wrapper = WildrGqlIsolateBlocWrapper.isolate(
      eventReceiver,
      onBLocClose,
  ...
  /// Creates [IsolateBlocBase] and connects it to the [WildrGqlIsolateBlocWrapper].
  ...
  void _closeBloc(String uuid) {
    ...
    if (bloc == null) {
      ...
      final wrappers =
          _isolatedBlocWrappers[bloc] ?? <WildrGqlIsolateBlocWrapper>[];
...
class BlocUnregisteredException implements Exception {
  ...
  @override
  String toString() =>
      'You are trying to create $_blocType which is not registered.\n'
...
void _print(String message) {
  // debugPrint('[IsolateManager] $message');
}
