...
import (
	...
	gl "github.com/wildr-inc/app/genesis/pkg/transport/grpc/limiter"
...
func TestSecureUnary(t *testing.T) {
	t.Skip("TLS not currently needed")
	Convey("Given I have a gRPC server", t, func() {
		lc := fxtest.NewLifecycle(t)
		logger := test.NewLogger(lc)
	...
	Convey("Given I have a gRPC server", t, func() {
		...
		Convey("When I query for an authenticated greet", func() {
			...
			conn := test.NewGRPCClient(
				...
				cfg,
				test.NewTracerConfig(),
				token.NewPerRPCCredentials(test.NewGenerator("test", nil)),
				m,
			)
			defer conn.Close()
	...
	Convey("Given I have a gRPC server", t, func() {
		...
		Convey("When I query for a unauthenticated greet", func() {
			...
			conn := test.NewGRPCClient(
				...
				cfg,
				test.NewTracerConfig(),
				token.NewPerRPCCredentials(test.NewGenerator("bob", nil)),
				m,
			)
			defer conn.Close()
	...
	Convey("Given I have a gRPC server", t, func() {
		...
		Convey("When I query for a unauthenticated greet", func() {
			...
			conn := test.NewGRPCClient(
				...
				cfg,
				test.NewTracerConfig(),
				token.NewPerRPCCredentials(test.NewGenerator("", nil)),
				m,
			)
			defer conn.Close()
	...
	Convey("Given I have a gRPC server", t, func() {
		...
		Convey("When I query for a unauthenticated greet", func() {
			...
			conn := test.NewGRPCClient(
				...
				cfg,
				test.NewTracerConfig(),
				token.NewPerRPCCredentials(
					test.NewGenerator("bob", errors.New("token error")),
				),
				m,
			)
			defer conn.Close()
...
func TestBreakerUnary(t *testing.T) {
	Convey("Given I have a gRPC server", t, func() {
		...
		Convey(
			"When I query for a unauthenticated greet multiple times",
			func() {
				ctx := context.Background()
				conn := test.NewGRPCClient(
					ctx,
...
func TestLimiterUnary(t *testing.T) {
	Convey("Given I have a gRPC server", t, func() {
		...
		gs := test.NewGRPCServer(
			...
			m,
			[]grpc.UnaryServerInterceptor{
				gl.UnaryServerInterceptor(l, meta.UserAgent),
			},
			[]grpc.StreamServerInterceptor{
				gl.StreamServerInterceptor(l, meta.UserAgent),
			},
		)
