...
func (h *Handler) ServeHTTP(
	...
	if d, ok := ctx.Deadline(); ok {
		fields = append(
			fields,
			zap.String(httpDeadline, d.Format(time.RFC3339)),
		)
	}
	...
	loggerLevel(
		fmt.Sprintf("finished call with code %s", res.Status()),
		fields...)
}
...
func (r *RoundTripper) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	if err != nil {
		...
		r.logger.Error("finished http call with error", fields...)
	...
	loggerLevel(
		fmt.Sprintf("finished call with code %s", resp.Status),
		fields...)
...
func codeToLevel(
	code int,
	logger *zap.Logger,
) func(msg string, fields ...zapcore.Field) {
