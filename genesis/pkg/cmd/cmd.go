...
func New(version string) *Command {
	...
	root.PersistentFlags().
		StringVar(&inputFlag, "input", "env:CONFIG_FILE", "input config location (format kind:location, default env:CONFIG_FILE)")
...
func (c *Command) AddServerCommand(
...
func (c *Command) AddClientCommand(
	name, description string,
	opts ...fx.Option,
) *cobra.Command {
	cmd := &cobra.Command{
